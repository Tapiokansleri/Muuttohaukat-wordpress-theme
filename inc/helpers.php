...
/**
 * Render a footer column from ACF footer menu row (legacy haukka data).
 *
 * @param array $menu_row Row with 'name' and 'menu' (post IDs).
 */
function renderFooterAcfMenuColumn(array $menu_row) {
  if (empty($menu_row['menu']) || !is_array($menu_row['menu'])) {
    return;
  }

  $title = !empty($menu_row['name']) ? $menu_row['name'] : '';
  ?>
  <div class="footer-widget footer-widget--acf-menu">
    <?php if ($title !== '') : ?>
      <h3 class="footer-widget__title"><?= esc_html($title) ?></h3>
    <?php endif; ?>
    <ul>
      <?php foreach ($menu_row['menu'] as $post_id) :
        $post_id = (int) $post_id;
        if (!$post_id) {
          continue;
        }
        ?>
        <li><a href="<?= esc_url(get_permalink($post_id)) ?>"><?= esc_html(get_the_title($post_id)) ?></a></li>
      <?php endforeach; ?>
    </ul>
  </div>
  <?php
