 ...
 *
 * The existing code may only be preserved when the operator has not changed
 * the destination or account id.
 *
 * @param string $endpoint Endpoint URL.
 * @return string
 */
function d365EndpointIdentity($endpoint) {
  ...
  return strtolower((string) ($parts['scheme'] ?? '')) . '://'
    ...
    . (isset($parts['port']) ? ':' . absint($parts['port']) : '')
    . rtrim((string) ($parts['path'] ?? ''), '/')
    . '?id=' . rawurlencode(is_scalar($query['id'] ?? null) ? (string) $query['id'] : '');
}

/**
 * Preserve the stored D365 code when the masked value is submitted.
 *
...
function sanitizeD365Endpoint($input) {
  ...
  if (
    ...
    || d365EndpointIdentity($endpoint) !== d365EndpointIdentity($current)
  ) {
    add_settings_error(
      'muuttohaukat_d365_endpoint',
      'muuttohaukat_d365_destination_changed',
      __('Kun D365-osoitetta tai id-parametria muutetaan, myös code-avain on syötettävä uudelleen.', 'muuttohaukat'),
      'error'
    );
  ...
  $current_query = [];
  parse_str((string) wp_parse_url($current, PHP_URL_QUERY), $current_query);
  if (!isset($current_query['code']) || !is_scalar($current_query['code']) || trim((string) $current_query['code']) === '') {
...
/**
 * Mask the D365 function code for safe admin rendering.
 *
 * @param string $endpoint Endpoint URL.
 * @return string
 */
function maskD365Endpoint($endpoint) {
  $query = [];
  parse_str((string) wp_parse_url($endpoint, PHP_URL_QUERY), $query);
  if (!isset($query['code']) || !is_scalar($query['code']) || trim((string) $query['code']) === '') {
    return $endpoint;
  }

  return add_query_arg('code', D365_CODE_MASK, $endpoint);
}
...
add_action('admin_notices', function () {
  if (!current_user_can('manage_options') || \Muuttohaukat\d365_endpoint_is_valid(\Muuttohaukat\d365_endpoint())) {
    return;
  }

  $url = add_query_arg(['page' => PAGE_SLUG, 'tab' => 'links'], admin_url('themes.php'));
  echo '<div class="notice notice-error"><p>';
  echo wp_kses_post(sprintf(
    __('D365-lomakevälitys ei ole käytössä. <a href="%s">Lisää täydellinen Azure Function -osoite ennen tuotantoon siirtoa.</a>', 'muuttohaukat'),
    esc_url($url)
  ));
  echo '</p></div>';
});
...
function renderD365Fields() {
  ...
  $display = maskD365Endpoint($display);
      ...
      <table class="form-table" role="presentation">
        <tr>
          ...
          <td>
            ...
            >
            <p class="description">
              <?= esc_html__('Osoitteen on oltava HTTPS Azure AddOfferToDynamics -URL, jossa on ei-tyhjät id- ja code-parametrit. Tallennettu code näytetään peitettynä.', 'muuttohaukat') ?>
            </p>
          </td>
        </tr>
        <tr>
          ...
          <td>
            ...
            <?php else : ?>
              <span style="color: #d63638;">
                <?= $effective === ''
                  ? esc_html__('Puuttuu — lomakkeet eivät välity Dynamics 365:een', 'muuttohaukat')
                  : esc_html__('Virheellinen — tarkista Azure-osoite sekä id- ja code-parametrit', 'muuttohaukat') ?>
              </span>
            <?php endif; ?>
          </td>
        </tr>
...
/**
 * Reduce a PHP log line to its theme marker and mask absolute paths.
 *
 * @param string $line Log line.
 * @return string
 */
function maskThemeLogLine($line) {
  $d365_pos = stripos($line, '[D365]');
  $form_pos = stripos($line, '[Theme form]');
  $positions = array_filter([$d365_pos, $form_pos], function ($position) {
    return $position !== false;
  });

  if (!empty($positions)) {
    $line = substr($line, min($positions));
  }
...
function renderErrorLog() {
  ...
  $d365_php_lines = array_values(array_filter($php_lines, function ($line) {
    return stripos($line, '[D365]') !== false
      || stripos($line, '[Theme form]') !== false;
  }));
  $d365_php_lines = array_map(__NAMESPACE__ . '\\maskThemeLogLine', $d365_php_lines);
  ?>
  <div class="card" style="max-width: 960px; margin-top: 1.5em; padding: 1em 1.25em;">
    ...
    <?php elseif (empty($d365_php_lines)) : ?>
      <p><em><?= esc_html__('D365- tai teeman lomakerivejä ei löytynyt.', 'muuttohaukat') ?></em></p>
    <?php else : ?>
      <p class="description">
        <?= esc_html(sprintf(__('Näytetään D365- ja teeman lomakerivit (viimeiset %d riviä).', 'muuttohaukat'), count($d365_php_lines))) ?>
      </p>
