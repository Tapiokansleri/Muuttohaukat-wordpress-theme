    ...
    return fetch(url, { signal: signal })
      .then(function (res) {
        return res.json().then(function (json) {
          if (!res.ok) {
            var error = new Error(json && json.message ? json.message : 'Post listing request failed.');
      ...
      .then(function (json) {
        if (!json || typeof json.html !== 'string') {
          throw new Error('Post listing response did not contain HTML.');
        }
        if (!signal || !signal.aborted) {
          var oldError = list.parentNode ? list.parentNode.querySelector('.mh-postlisting__error') : null;
    ...
    function updateList() {
      ...
      fetchPosts(listEl, query, template, abortController.signal)
        ...
        .catch(function (e) {
          ...
          var errorEl = listing.querySelector('.mh-postlisting__error');
          if (!errorEl) {
            errorEl = document.createElement('p');
            errorEl.className = 'mh-postlisting__error';
            errorEl.setAttribute('role', 'alert');
            listing.insertBefore(errorEl, listEl);
          }
