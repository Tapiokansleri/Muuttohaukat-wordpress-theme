...
Alternatively, add it to `wp-config.php`. A saved admin value takes priority:
...
- Theme-native breadcrumbs and site-wide comment disabling
- Sanitized administrator/editor SVG uploads
- Legacy `[query_pages_by_category]` and hardened PostListing compatibility

## Utility plugin consolidation and rollback

When Muuttohaukat is activated, it deactivates Query Pages by Category, Disable Comments, SVG Support, Syllable Hyphenator, WP Query Route to REST API, WPCodeBox 2, and Breadcrumb NavXT after their required site-specific behavior has moved into the theme. Plugin files and settings are retained; this migration does not delete plugin data.

If you roll back to Haukka, reactivate the required legacy plugins. In particular, Haukka's AJAX PostListing depends on **WP Query Route to REST API** and will not work until that plugin is active again. Reactivate Breadcrumb NavXT, SVG Support, Disable Comments, and the other utility plugins when the corresponding legacy behavior is needed.

The Google Search Console verification value is stored in The SEO Framework settings. It is intentionally never committed to this public repository.
